...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='./runs/train/yolov5n_bird_night_white/weights/best.pt', help='model path or triton URL')
    parser.add_argument('--source', type=str, default= '0', help='file/dir/URL/glob/screen/0(webcam)')
    parser.add_argument('--data', type=str, default='./data/drone_data_bird_night_white.yaml', help='(optional) dataset.yaml path')
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    y = (y1 + y2) / 2
                    point = str(x) + " " + str(y)
                    data_record.record(x,y,coord_record)
                    emptyflag = 0
            ...
            if (len(data) == 3):
                filtered_coord = kalman_filter(data)
                x_out = round(filtered_coord[0],1)
                y_out = round(filtered_coord[1],1)
                flagf = 1
                data_str = f"{x_out},{y_out},{flagf}\n"
