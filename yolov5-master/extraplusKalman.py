...
from kalman_test import kalman_filter


def parse_opt():
    parser = argparse.ArgumentParser()
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'E:/python code/FSO-UAVtest/yolov5-master/runs/train/yolov5n-allin_bird/weights/best.pt', help='model path or triton URL')
...
def Initial(
    ...
    return model,dataset,names,pt,bs
...
def inference(
    ...
    arduino = serial.Serial('COM7', 9600)  # 替换为正确的串口号
    time.sleep(2)
    data = []
    view_img = check_imshow(warn=True)
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    cv2.putText(im0, '+', (int((x1 + x2) / 2), int((y1 + y2) / 2)), cv2.FONT_HERSHEY_SIMPLEX, 1,
                                (0, 0, 255), 4)
                    coord_temp = ((x,y),True)
                    data.append(coord_temp)

            else:
                print("no detections")
                coord_temp = ((320,240),False)
                data.append(coord_temp)

            if (len(data) == 5):
                all_flag_false = all(flagF == False for _,flagF in data)
                if all_flag_false:
                    x_default = 320
                    y_default = 240
                    flagf = 0
                    data_str_default = f"{x_default},{y_default},{flagf}\n"
                    arduino.write(data_str_default.encode())
                    time.sleep(0.02)
                    print(data_str_default)
                else:
                    ...
                    for item in filtered_data:
                        if item is not None:
                            ...
                            aveoriy = []
                            (x_list,y_list) = item
                            aveorix.append(x_list[0])
                            aveoriy.append(y_list[0])
                    flagf = 1
                    ave_x = sum(aveorix)/len(aveorix)
                    ave_y = sum(aveoriy)/len(aveoriy)
                    coord_str = f"{ave_x},{ave_y},{flagf}\n"
